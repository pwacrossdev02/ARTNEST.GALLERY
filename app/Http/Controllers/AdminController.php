...
class AdminController extends Controller
{
    ...
    public function top_category_products_section(Request $request)
    {
        $top_categories_products = DB::table(DB::raw('(SELECT products.id product_id, products.name product_name, products.slug product_slug, products.auction_product, products.category_id,
                                                        `products`.`thumbnail_img` as `product_thumbnail_img`,`products`.`certificate_img ` as `certificate_img `, od.sales, od.total, od.created_at order_detail_created,
            ...
            ->select(DB::raw('category_id, category_name, cover_image, product_id, product_name, product_slug, auction_product, product_thumbnail_img,certificate_img , sales, total, order_detail_created'))
            ->where('rn', '<=', 3)
            ->orderBy('sales', 'desc')
            ->get();
        ...
        foreach ($top_categories_products as $key => $row) {
            ...
            $row->certificate_img = Upload::where('id', $row->certificate_img )->first();
            $row->thumbnail_img = Upload::where('id', $row->thumbnail_img)->first();
    ...
    public function top_sellers_products_section(Request $request)
    {
        ...
        foreach ($new_top_sellers as $key => $row) {
            ...
            $products_query->select('products.id AS product_id', 'products.name', 'products.slug AS product_slug', 'products.auction_product', 'products.thumbnail_img','products.certificate_img', DB::raw('SUM(quantity) AS total_quantity, SUM(price * quantity) AS sale'))
    ...
    public function top_brands_products_section(Request $request)
    {
        $top_brands_products = DB::table(DB::raw('(SELECT products.id product_id, products.name product_name, products.slug product_slug, products.auction_product, products.brand_id,
                                                        `products`.`thumbnail_img` as `product_thumbnail_img`,`products`.`certificate_img` as `certificate_img`, od.sales, od.total, brands.name AS brand_name,
                                                        `brands`.`logo`,
            ...
            ->select(DB::raw('brand_id, brand_name, logo, product_id, product_name, product_slug, auction_product, product_thumbnail_img,certificate_img, sales, total'))
            ->where('rn', '<=', 3)
            ->orderBy('total', 'desc')
