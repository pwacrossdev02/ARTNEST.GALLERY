...
class DemoController extends Controller
{
    ...
    public function convert_assets(Request $request)
    {
        ...
        foreach (CustomerProduct::all() as $key => $product) {
            ...
            if ($product->certificate_img != null) {
                $arr = explode('.', $product->certificate_img);
                $upload = Upload::create([
                    'file_original_name' => null, 'file_name' => $product->certificate_img, 'user_id' => $product->user_id, 'extension' => $arr[1],
                    'type' => isset($type[$arr[1]]) ?  $type[$arr[1]] : "others", 'file_size' => 0
                ]);
