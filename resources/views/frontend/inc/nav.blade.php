                    ...
                    <div class="d-none d-xl-block ml-auto mr-0">
                        ...
                        @else
                            ...
                            <span class="d-flex align-items-center nav-user-info ml-3">
                                ...
                                <a href="{{ route('user.registration') }}" class="text-reset opacity-60 hov-opacity-100 hov-text-primary fs-12 d-inline-block border-right border-soft-light border-width-2 pr-2 ml-3">{{ translate('Registration') }}</a>
                        @if (get_setting('vendor_system_activation') == 1 && !Auth::check() )
                               
                                <a href="{{ route('seller.product.products.create_productnewseller') }}" class="text-reset opacity-60 hov-opacity-100 hov-text-primary fs-12 d-inline-block py-2 pl-2">{{ translate('Seller') }}</a>
                        @endif
                            ...
                            <ul class="list-inline mb-0 pl-0 hor-swipe c-scrollbar-light">
                                @if (get_setting('header_menu_labels') != null)
                                    @foreach (json_decode(get_setting('header_menu_labels'), true) as $key => $value)
                                        ...
                                        {{-- Skip items with "blog" in the link or label --}}
                                        @if (!str_contains(strtolower($value), 'blog') && !str_contains(strtolower($link), 'blog'))
                                            <li class="list-inline-item mr-0 animate-underline-white">
                                                <a href="{{ $link }}"
                                                class="fs-13 px-3 py-3 d-inline-block fw-700 {{ $nav_txt_color }} header_menu_links hov-bg-black-10
                                                @if (url()->current() == $link) active @endif">
                                                    {{ translate($value) }}
                                                </a>
                                            </li>
                                        @endif
