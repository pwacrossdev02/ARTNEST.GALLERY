                            ...
                            @foreach ($sliders as $key => $slider)
                                <div class="carousel-box">
                                    <a href="{{ isset(json_decode($home_slider_links, true)[$key]) ? json_decode($home_slider_links, true)[$key] : '' }}">
                                        <!-- Image -->
                                        <img class="d-block mw-100 img-fit overflow-hidden h-180px h-md-320px h-lg-315px overflow-hidden"
...
@if (count($featured_categories) > 0)
    <section class="custom-featured-categories mb-2 mb-md-3 mt-2 mt-md-3">
        <div class="container">
            <div class="bg-white p-2 rounded shadow-sm">
                <!-- Top Section -->
                <div class="d-flex mb-2 mb-md-3  justify-content-between align-items-center">
                    <!-- Title -->
                    <h3 class="fs-16 fs-md-20 fw-700 mb-2 mb-sm-0" style="    text-wrap: nowrap;">
                        <span>{{ translate('Featured Categories') }}</span>
                    </h3>
                    <!-- Links -->
                    <div class="d-flex justify-content-center justify-content-md-end w-100">
                        <a class="custom-view-all-categories text-blue fs-10 fs-md-12 fw-700 hov-text-primary animate-underline-primary"
                            href="{{ route('categories.all') }}">{{ translate('View All Categories') }}</a>
                    </div>
                </div>
            </div>
            <!-- Categories -->
            <div class="bg-white px-3 rounded shadow-sm">
                <div class="row">
                    @foreach ($featured_categories->take(6) as $key => $category)
                    @php
                        $category_name = $category->getTranslation('name');
                    @endphp
                        <div class="col-xl-3 col-md-4 py-3 py-md-2rem">
                            <div class="custom-category-item position-relative h-100">
                                <!-- Image with overlay for text -->
                                <div class="custom-category-img-wrapper mb-3 mb-sm-0 h-100">
                                    <img src="{{ isset($category->bannerImage->file_name) ? my_asset($category->bannerImage->file_name) : static_asset('assets/img/placeholder.jpg') }}"
                                        class="lazyload w-100 h-100 mx-auto has-transition rounded-3 shadow-sm custom-category-img"
                                        alt="{{ $category->getTranslation('name') }}"
                                        onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';">
                                    <!-- Overlay Text with link -->
                                    <div class="custom-category-text-overlay">
                                        <h5 class="text-white mb-0">
                                            <a href="{{ route('products.category', $category->slug) }}" class="text-white text-decoration-none">{{ $category_name }}</a>
                                        </h5>
                                    </div>
                                </div>
                                <!-- <div class="custom-category-links px-2 px-lg-4">
