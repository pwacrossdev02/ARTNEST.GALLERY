<div class="text-left">
    ...
    @if ($detailedProduct->brand != null)
        <div class="d-flex flex-wrap align-items-center mb-3">
            <span class="text-secondary fs-14 fw-400 mr-4 w-80px">{{ translate('Artists') }}</span><br>
            <a href="{{ route('products.brand', $detailedProduct->brand->slug) }}"
                class="text-reset hov-text-primary fs-14 fw-700">{{ $detailedProduct->brand->name }}</a>
        </div>
    @endif
    @if ($detailedProduct->width != null || $detailedProduct->height != null || $detailedProduct->depth != null)
        <div class="d-flex flex-wrap mb-2">
            <span class="text-secondary fs-14 fw-400 w-80px">{{ translate('Dimensions') }}:</span>
            <div class="text-dark fw-700">
                <span class="ml-1"> {{ translate('Width') }}: {{ $detailedProduct->width }} cm</span>
                <span class="ml-3">{{ translate('Height') }}: {{ $detailedProduct->height }} cm</span>
                <span class="ml-3">{{ translate('Depth') }}: {{ $detailedProduct->depth }} cm</span>
            </div>
        </div>
    @endif
    <!-- Seller Info -->
